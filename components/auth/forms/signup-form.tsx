...
export function SignupForm({
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    console.log("📝 [SIGNUP FORM] Form values:", { 
      email: values.email, 
    ...
    try {
      ...
      console.log("📡 [SIGNUP FORM] Server response:", { success, message });

      if (success) {
        console.log("✅ [SIGNUP FORM] Signup successful, redirecting to verify page");
        toast.success(
          `${message as string} Please check your email for verification. The link will expire in 15 minutes.`
        );
        router.push(`/verify?email=${encodeURIComponent(values.email)}`);
      } else {
        console.error("❌ [SIGNUP FORM] Signup failed:", message);
        toast.error(message as string);
      }
    } catch (error) {
      console.error("❌ [SIGNUP FORM] Exception during signup:", error);
